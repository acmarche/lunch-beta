...
class CommerceVoter extends Voter
{
    ...
    protected function supports($attribute, $subject)
    {
        ...
        return $subject instanceof CommerceInterface && in_array(
                $attribute,
                [self::ADD_SUPPLEMENT, self::ADD_PRODUIT, self::ADD_INGREDIENT, self::SHOW, self::EDIT, self::DELETE]
            );
    }
    ...
    private function canView(CommerceInterface $commerce, TokenInterface $token)
    {
        ...
        if ($this->decisionManager->decide($token, ['ROLE_LUNCH_LOGISTICIEN']))
            return true;
    ...
    {
        if ($this->decisionManager->decide($token, ['ROLE_LUNCH_COMMERCE'])) {
            $user = $token->getUser();
